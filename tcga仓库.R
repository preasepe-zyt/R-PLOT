...
setwd("xena")
library(tidyverse)
exp <- read.csv("biaoda1.csv")
DEG <- read.csv("DEG2.csv")

exp <- exp[!duplicated(exp$Genes),]
rownames(exp) <- exp$Genes
exp = exp[,-1]

DEG <- DEG[!duplicated(DEG$Genes),]
rownames(DEG) <- DEG$Genes
DEG = DEG[,-1]


exp <- read.table("biaoda1.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)

DEG <- read.table("DEG2.txt",sep = "\t",row.names = 1,check.names = F,stringsAsFactors = F,header = T)
...
exp_diff <- exp[cg,]
group_list=factor(ifelse(substr(colnames(exp),1,6) == "01A","T","N"),levels = c("N","T"))
annotation_col=data.frame(group=group_list)

write.csv(annotation_col,"annotation_col.csv",sep = "\t",row.names = T,col.names = NA,quote = F)
annotation_col <- read.csv("annotation_col.CSV")
rownames(annotation_col) <- annotation_col$X
annotation_col = annotation_col[,-1]

rownnames(annotation_col)=colnames(exp_diff)
pheatmap(exp_diff,
         annotation_col=annotation_col,
         scale = "row",
         show_rownames = F,
...
ggscatter(DEG, x = "log2FoldChange", y = "logP", xlab = "log2FoldChange",
          ...
          size = 1) +
  theme_base() +
  geom_hline(yintercept = -log10(0.3), linetype = "dashed") +
...
#高表达的基因中，选择fdr值最小的5个
up.genes <- head(DEG$Genes[which(DEG$change == "UP")], 5)
#低表达的基因中，选择fdr值最小的5个
down.genes <- head(DEG$Genes[which(DEG$change == "DOWN")], 5)
#将up.genes和down.genes合并，并加入到Label中
DEG.top5.genes <- c(as.character(up.genes), as.character(down.genes))
DEG$Label[match(DEG.top5.genes, DEG$Genes)] <- DEG.top5.genes

write.csv("DEG.csv")

write.csv(DEG,"DEG2.csv",sep = "\t",row.names = T,col.names = NA,quote = F)
